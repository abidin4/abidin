...
def colors(value):
    patterns = {
        'R1' : '\033[96;1m', 'R2' : '\033[31;2m',
        'G1' : '\033[94;1m', 'Y1' : '\033[33;1m',
        'P1' : '\033[91;1m', 'CC' : '\033[0m'
    }
...
def log(value, status='PERHATIAN PERHATIAN', color='[R2]'):
...
def log_replace(value, status='>>>>>>>>>Menyuntikkan Anuh<<<<<<<<<', color='[R1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/amza.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('amza.txt', color='[R1]')
                return
            self.log('Jangan lupa cari anuh yg keci ya: 127.0.0.1=8080'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(65535)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('jangan lupa cuci tangan dan cuci muka.'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='>>>>>kopinya<<<<<<', color='[P1]'):
    ...
    def run(self):
        try:
            ...
            self.log('seruput kopinya '.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
            self.log('--------gas lagi---------', color='[R1]')
        except TimeoutError:
            self.log('{} not responding'.format(self.proxy_host), color='[R1]')

def main():
    print(colors('\n'.join([
        '[G1]DiGUNAKN UNTUK ANUH YA GAN',
        '[Y1]SCRIPT BAGI YG JOMBLO, TETAP DIRUMAH YA', '[R1]'
    ])))
